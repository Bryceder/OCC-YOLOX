...
class YoloDataset(Dataset):
    def __init__(self, annotation_lines, input_shape, num_classes, epoch_length, \
                        mosaic, mixup, mosaic_prob, mixup_prob, erase, erase_prob, train, special_aug_ratio = 0.7):
    ...
    def __getitem__(self, index):
        ...
        if self.mosaic and self.rand() < self.mosaic_prob and self.epoch_now < self.epoch_length * self.special_aug_ratio:
            ...
            if self.erase < self.erase_prob:
                image      = self.get_random_eraser(image)
    ...
    def get_random_eraser(p=0.5, s_l=0.02, s_h=0.4, r_1=0.3, r_2=1/0.3, v_l=0, v_h=255, pixel_level=True):
        def eraser(image):
            if image.ndim == 3:
                img_h, img_w, img_c = image.shape[:3]
            elif image.ndim == 2:
