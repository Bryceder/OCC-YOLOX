...
class GSConv(nn.Module):
    def __init__(self, in_channels, out_channels, ksize=1, stride=1, groups=1, act="silu"):
        super().__init__()
        c_ = out_channels // 2
        self.cv1 = BaseConv(in_channels, out_channels // 2, ksize=ksize, stride=stride, groups=groups, act=act)
        self.cv2 = BaseConv(out_channels // 2, out_channels // 2, stride=1, groups=groups, ksize=ksize, act=act)
    def forward(self, x):
        x1 = self.cv1(x)
...
class CSPDarknet(nn.Module):
    def __init__(self, dep_mul, wid_mul, out_features=("dark3", "dark4", "dark5"), depthwise=False, act="silu",):
        ...
        self.dark5 = nn.Sequential(
            DeformConv2d(base_channels * 8, base_channels * 16, 3, 1, 2, modulation=True),
            SPPFBottleneck(base_channels * 16, base_channels * 16, activation=act),
            AttentionCSPLayer(base_channels * 16, base_channels * 16, n=base_depth, shortcut=False, depthwise=depthwise, act=act),
        )
