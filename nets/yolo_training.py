...
class IOUloss(nn.Module):
    ...
    def forward(self, pred, target):
        ...
        if self.loss_type == "iou":
            ...
            with torch.no_grad():
                alpha = v / ((1 + 1e-7) - iou + v)
            h = max(0, min(pred[2], target[2]) - max(pred[0], target[0]))
            w = max(0, min(pred[3], target[3]) - max(pred[1], target[1]))

            w_ = torch.min((pred[2]), (target[2]))
            h_ = torch.min((pred[3]), (target[3]))
            w_1 = torch.min((pred[0]), (target[0]))
            h_1 = torch.min((pred[1]), (target[1]))
            w_2 = torch.max((pred[0]), (target[0]))
            h_2 = torch.max((pred[1]), (target[1]))
            w_3 = torch.min((pred[2]), (target[2]))
            h_3 = torch.min((pred[2]), (target[2]))
            w_4 = torch.max((pred[3]), (target[3]))
            h_4 = torch.max((pred[3]), (target[3]))

            v_2 = (4 / math.pi ** 2) * torch.pow(torch.atan(w/ torch.clamp(c_br[:, 0]-c_tl[:, 0], min = 1e-7)) - 
                                                torch.atan(h/ torch.clamp(c_br[:, 1]-c_tl[:, 1], min = 1e-7)), 2)
            with torch.no_grad():
                beta = v_2 / ((1 + 1e-7) - iou + v_2)
            ...
            oliou = iou - (center_dis / convex_dis) - (dis_w / C_w) - (dis_h / C_h) - alpha * v - beta * v_2
            loss = 1 - oliou.clamp(min=-1.0, max=1.0)
            ...
            loss = 1 - eiou.clamp(min=-1.0, max=1.0)  # eiou loss

            loss = (iou**2)*loss.clamp(min=-1.0, max=1.0)  # focal_eiou loss
...
class YOLOLoss(nn.Module):    
    def __init__(self, num_classes, fp16, strides=[8, 16, 32]):
        ...
        #self.iou_loss           = NMSLoss(reduction='none', loss_weight=1.0, use_score = True, add_gt = False, pull_relax = False, push_relax = True, push_select = True, fix_pushs_grad = False, fix_reg_gradient=False, fix_pull_reg=False, pull_weight = 1, push_weight = 1, nms_thr = 0.5)
